...
import { TechIssueFormData } from '@common/types';

type Props = {
    ticketFormData?: TechIssueFormData & { id: string }
}

const { ticketFormData } = defineProps<Props>()

const ruleFormRef = ref<FormInstance>()
const ruleForm = reactive<TechIssueFormData>(ticketFormData || {
    area: 'Account',
    issue: '',
    severity: 'S4',
    ticket: '',
...
const rules = reactive<FormRules>({
    ...
    ticket: [{required: true, message: 'Technical Issue must input an incident url',}],
    ...
    await formEl.validate(async (valid, fields) => {
        if (valid) {
            if (ticketFormData?.id) {
                await updateTicket(ticketFormData.id, 'techIssue', ruleForm);
                ElMessage({
                    type: 'success',
                    message: 'Update success!'
                })
            } else {
                await createTicket('techIssue', ruleForm);
                ElMessage({
                    type: 'success',
